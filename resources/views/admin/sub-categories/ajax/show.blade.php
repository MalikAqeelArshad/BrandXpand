<form method="POST" action="{{ route('sub-categories.update', $subCategory->id) }}">
    ...
    <div class="modal-body px-4">
        ...
        <div class="form-group">
            <label>Name</label>
            <input type="text" name="name" value="{{ $subCategory->name }}" class="form-control" placeholder="Enter the name of category" required>
        </div>
        <div class="form-group">
            <label>Description</label>
            <textarea name="description" class="form-control" placeholder="Write a short description about the category...">{{ $subCategory->description }}</textarea>
        </div>
    </div>
