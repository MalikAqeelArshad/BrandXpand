...
<main class="content">
    ...
    <ol class="breadcrumb shadow-sm small">
        <li class="breadcrumb-item">
            <a href="{{ route('order.notifications') }}"><i class="fa fa-lg fa-bell fa-fw"></i> Notifications</a>
        </li>
        <li class="breadcrumb-item"><a href="javascript:;">Order</a></li>
        <li class="breadcrumb-item active">{{ $orders->first()->reference_number}}</li>
    </ol>
    <!--end:: Breadcrumb -->
    ...
    <!--begin:: Order List -->
    <ul class="list-group list-group-flush size-md shadow-sm mt-4">
        ...
        @role(['administrator', 'admin'])
        <li class="list-group-item border-light p-0">
            <form method="POST" action="{{ route('orders.update', $firstOrder->reference_number) }}">
                @csrf @method('PUT')
                <div class="d-sm-flex align-items-center justify-content-end alert alert-secondary rounded-0 m-0 border-white">
